...
import getFats from "@salesforce/apex/FTTHLeafletMapController.getFats";
...
export default class ftthleafletMap extends LightningElement {
  @track showMap = false;
  @track mapResult = {
  ...
  @wire(getFats, { siteId: "$recordId" })
  FatsWireHandler({ error, data }) {
    if (data) {
        console.log("Fats data:", data);
        this.Fats = data;
        this.refreshMap();
    } else if (error) {
  ...
  drawMap() {
    ...
    console.log("Fats data for drawing map:", this.Fats);
    ...
    if (this.Fats) {
        ...
        console.log("Entered 5",this.Fats);

      for (let Fat of this.Fats) {
        const FatLat =
          Fat.location__Latitude__s;
        const FatLong =
          Fat.location__Longitude__s;
        const nominalDistance = Fat.Distancefromnominal__c;
        const popupText =
          "<p>Fat: " +
          Fat.Name +
        ...
        //   FatLong
        // );
        const marker = L.marker([FatLat, FatLong], {
  ...
  refreshMap() {
    ...
    if (this.mapResult.map && this.siteRecord && this.Fats) {
